...
class EventItem extends Component {
  ...
  componentDidUpdate(prevProps) {
    // Check for actual changes to prevent infinite loops
    ...
    // Only resubscribe to resize events if necessary props changed
  ...
  render() {
    ...
    if (config.dragAndDropEnabled) {
      // Create a ref wrapper component
      const DragWrapper = React.forwardRef((props, ref) => (
        <div ref={ref}>{props.children}</div>
      ));
      DragWrapper.displayName = 'DragWrapper';
      
      // Apply both refs using the wrapper
      aItem = (
        <DragWrapper ref={node => {
