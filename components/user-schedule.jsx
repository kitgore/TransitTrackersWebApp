    ...
    const loadShifts = async () => {
      ...
      try {
        ...
        if (firebaseShifts.length > 0) {
          ...
          const formattedShifts = firebaseShifts.map(shift => {
            ...
            try {
              ...
              console.log('✅ Formatted shift:', {
                ...
                role: formattedShift.role,
                group: formattedShift.group
              });
          ...
          // Force a redraw after setting shifts
          if (timelineRef.current) {
            console.log('Forcing immediate timeline redraw');
            timelineRef.current.redraw();
          }
        } else {
...
const [options] = useState({
  ...
  end: new Date(new Date(currentDate).setHours(16, 0, 0, 0)),  // initial view end
...
useEffect(() => {
  if (shifts.length > 0 && roles.length > 0 && timelineRef.current) {
    console.log('Data ready, forcing timeline reinitialization');
    // Force a complete redraw and recalculation
    if (timelineRef.current.dom && timelineRef.current.dom.root) {
      ...
      console.log('Timeline DOM not ready yet, retrying in 100ms');
      setTimeout(() => {
...
const getTimelineRef = (ref, itemsDataset, groupsDataset) => {
  console.log('Timeline mounted with:', {
    ref: !!ref,
    itemsDataset: !!itemsDataset,
    groupsDataset: !!groupsDataset,
    shifts: shifts.length,
    roles: roles.length,
    hasDom: ref && ref.dom ? true : false
  });
  ...
  if (ref) {
    ...
    // Force initial redraw with a delay to ensure DOM is ready
    const initializeTimeline = () => {
      if (ref && ref.dom && ref.dom.root) {
        console.log('Forcing initial timeline redraw');
        const container = ref.dom.root;
        const height = container.clientHeight;
        const width = container.clientWidth;
